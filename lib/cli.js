...
function fixTypo(input, output) {
  ...
  if (output.alternativeMatches != null) {
    match = match + chalk.dim(" ↔ " + output.alternativeMatches.join(", "));
  }
  ...
  try {
    if (input != output.match) {
      ...
      sentence = sentence.replace(new RegExp(escaped, 'g'), 
        chalk.inverse(output.match));

      console.error('✓ ' + input + chalk.dim(' → ') + match);
    } else {
      console.error(chalk.dim('✓ ') + input + chalk.dim(' → ' + match));
    }
    console.error(chalk.dim(output.help));
  } catch (err) {
    console.error('✗ ' + input + chalk.dim(' → ') + match);
    console.error(chalk.dim(output.help));
  }
