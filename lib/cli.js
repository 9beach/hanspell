...
function checkAll() {
  ...
  hanspell.spellCheckByDAUM(input, HTTP_TIMEOUT, fixTypos, () =>
    hanspell.spellCheckByPNU(input, HTTP_TIMEOUT, fixTypos, writeCorrectSentence),
  );
}
...
function readAndCheck(check) {
  try {
    let contents = fs.readFileSync(`${homedir}/.hanspell-ignore`, 'utf8');
