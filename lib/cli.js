...
// fixes a typo, and prints a log
function fixTypo(input, output) {
  // FIXME: need to avoid ambiguous replacements
  var found = 0;
  var replaced = false;

  if (input != output.match) {
...
function checkDAUM() {
  hanspell.checkSpellWithDAUM(sentence, HTTP_TIMEOUT, fixTypos, sayGoodbye);
}

function checkPNU() {
  hanspell.checkSpellWithPNU(sentence, HTTP_TIMEOUT, fixTypos, sayGoodbye);
}

function checkAll() {
  const input = sentence;
  hanspell.checkSpellWithDAUM(input, HTTP_TIMEOUT, 
    fixTypos, function() {
      hanspell.checkSpellWithPNU(input, HTTP_TIMEOUT, 
        fixTypos, sayGoodbye);
    });
}
...
function checkJoint() {
  ...
  hanspell.checkSpellWithPNU(sentence, HTTP_TIMEOUT, buildMap, function() {
    hanspell.checkSpellWithDAUM(sentence, HTTP_TIMEOUT, fixJointTypos, 
      sayGoodbye);
  });
}
